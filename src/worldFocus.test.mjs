...
test('every framing is a real oblique standoff, not a nadir or an inside-out sphere', () => {
  ...
  assert.deepEqual(kinds.sort(), ['feature', 'fire', 'vessel', 'volume']);
  for (const kind of kinds) {
    ...
    // A ground feature stays close-in; an airspace volume is tens of km wide
    // and has to be framed from far enough away to read as a volume.
    const rangeCeiling = kind === 'volume' ? 100000 : 10000;
    assert.ok(
      framing.rangeM <= rangeCeiling,
  ...
  assert.deepEqual(
    { ...WORLD_FOCUS_FRAMING.feature },
    { radiusM: 600, rangeM: 5000, pitchDeg: -40 },
  );
  assert.deepEqual(
    { ...WORLD_FOCUS_FRAMING.volume },
    { radiusM: 20000, rangeM: 70000, pitchDeg: -35 },
  );
});
